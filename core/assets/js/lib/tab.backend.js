window.paypalParty.tab = {};

window.paypalParty.tab.init = function() {
	window.paypalParty.tab.event();
};

window.paypalParty.tab.event = function() {
  jQuery( document ).on( 'click', '.tab-element', window.paypalParty.tab.load );
};

window.paypalParty.tab.load = function( event ) {
  ...
  event.preventDefault();
	...
	if ( ! tabTriggered.hasClass( 'no-tab' ) && tabTriggered.data( 'action' ) ) {
		...
		jQuery.post( window.ajaxurl, data, function( response ) {
			...
			window.paypalParty.tab.callTabChanged();
		} );
...
window.paypalParty.tab.callTabChanged = function() {
	var key = undefined;
	for ( key in window.paypalParty ) {
		if ( window.paypalParty[key].tabChanged ) {
			window.paypalParty[key].tabChanged();
		}
	}
};
